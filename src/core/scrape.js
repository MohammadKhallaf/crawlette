...
import { normalizeUrl, getBaseDomain, isExternalUrl, isSocialMediaUrl, SOCIAL_MEDIA_DOMAINS } from './normalize.js';
...
function parseSrcset(value) {
  ...
  return value.split(',').map((part) => {
    ...
    return { url: tokens[0], width: Number.isFinite(w) ? w : null };
  }).filter(Boolean);
}
 ...
 */
function closestUsefulText(el, minWords = 1) {
 ...
 */
function scoreImage(img, index, total) {
  let score = 0;
...
function imageUrlVariants(img) {
  const out = [];
  const push = (url, width = null) => {
...
function stripAttributes(root, keepDataAttributes) {
 ...
 *
 * @param {Document} doc   parsed document; MUTATED in place, pass a clone to keep the original
 * @param {string}   url   the page's own URL, used to resolve relative links
 * @returns {{cleanedHtml, links, media, tables, metadata}}
 */
export function scrape(doc, url, options = {}) {
  const {
    excludedTags = [],
    excludedSelector = '',
    cssSelector = null,
  ...
  if (cssSelector) {
    ...
    if (matches.length) {
      ...
      matches.forEach((m) => contentEl.appendChild(m.cloneNode(true)));
    }
  }
  ...
  return {
    cleanedHtml: contentEl.innerHTML.trim(),
...
function extractTables(root) {
  ...
  for (const table of root.querySelectorAll('table')) {
    ...
    let headers = [...rows[0].querySelectorAll('th')].map((c) => (c.textContent || '').trim());
    const bodyStart = headers.length ? 1 : 0;
    if (!headers.length) {
      headers = [...rows[0].querySelectorAll('td')].map((c) => (c.textContent || '').trim());
    }
