 ...
 */
function compositeScore(el, { minWordThreshold }) {
  ...
  const tagLen = (el.innerHTML || '').length;
...
function thresholdFor(el, { threshold, thresholdType }) {
...
function isPreserved(el, { preserveTags, preserveClasses }) {
 ...
 *
 * @returns {string} HTML of the surviving top-level blocks, joined.
 */
export function pruningFilter(doc, options = {}) {
  const {
    threshold = 0.48,
  ...
  const opts = { threshold, thresholdType, minWordThreshold, preserveTags, preserveClasses };
...
export const _internals = { compositeScore, thresholdFor, TAG_WEIGHTS, METRIC_WEIGHTS };
