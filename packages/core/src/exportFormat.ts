 ...
 * captured calls; nothing here issues a request. Calls are captured by
 * whatever host embeds this package (the Crawlette extension does it during a
 * Pick & record session); this module only knows how to format them.
 */
...
function apiCallMarkdown(call: ApiCall): string {
  const hints: string[] = [];
 ...
 */
export function apiCallsMarkdown(apiCalls: ApiCall[] | null | undefined): string {
...
export interface ExportedApiCall {
  method: string;
  url: string;
  itemCount: number;
  itemKey: string | null;
...
function apiCallForExport(call: ApiCall): ExportedApiCall {
  return {
    method: call.method,
    url: call.url,
...
export interface WithDiscoveredApis<T> {
  data: T;
  discoveredApis: ExportedApiCall[];
}
 ...
 */
export function withDiscoveredApis<T>(
  payload: T,
  apiCalls: ApiCall[] | null | undefined,
): T | WithDiscoveredApis<T> {
